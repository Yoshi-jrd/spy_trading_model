...
def train_and_stack_models(spy_data, interval_name):
    ...
    # Train other models
    rf_model = build_random_forest().fit(X_train_seq, y_train_seq.ravel())
    gb_model = build_gradient_boosting().fit(X_train_seq, y_train_seq.ravel())
    xgb_model = build_xgboost().fit(X_train_seq, y_train_seq.ravel())
    et_model = build_extra_trees().fit(X_train_seq, y_train_seq.ravel())
    cb_model = build_catboost().fit(X_train_seq, y_train_seq.ravel())

    # Stack predictions
    base_predictions = [
        rf_model.predict(X_train_seq),
        gb_model.predict(X_train_seq),
        xgb_model.predict(X_train_seq),
        et_model.predict(X_train_seq),
        cb_model.predict(X_train_seq),
        lstm_predictions
    ]
